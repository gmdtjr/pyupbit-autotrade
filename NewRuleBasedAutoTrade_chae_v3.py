...
while True:
    try:
        ...
        for i in range(1,coin_num+1):
            kkk[i-1], state[i-1], buy_price[i-1] = strategy(ticker_list[i-1], tic_list[i-1], kkk[i-1], state[i-1], buy_price[i-1], ratio)
        ...
        for i in range(1,coin_num+1):
            print(tic_list[i-1],'-','st:',state[i-1],'/k:',kkk[i-1],'/bp:',buy_price[i-1])
