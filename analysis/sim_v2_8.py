...
def market_manager(ticker, market_price, load_day):

    # Data Loading
    market_price_new = get_current_price(ticker, load_day)

    if (market_price != 0):
        price_ratio = 100*(market_price_new - market_price)/market_price
        if (price_ratio > 0):
            price_cnt = 1
        elif (price_ratio < 0):
            price_cnt = -1
    ...
    market_price = market_price_new

    return price_ratio, price_cnt, market_price

def strategy(ticker, kkk, hhh, state, buy_price, low_mean, high_min, low_min, load_day, cash, btc, stop_flag):
    ...
    return kkk, hhh, state, buy_price, current_price, cash, btc
...
while (not (now.year == 2022 and now.month == 2 and now.day == 15)):
    ...
    if (now.minute != minute_pre):
        ...
        price_cnt_sum = 0
        for i in range(1,coin_num+1):
            price_cnt_sum = price_cnt_sum + price_cnt[i-1]

        price_cnt_10[0] = price_cnt_sum
        price_cnt_10[1] = price_cnt_10[0]
        price_cnt_10[2] = price_cnt_10[1]
        price_cnt_10[3] = price_cnt_10[2]
        price_cnt_10[4] = price_cnt_10[3]
        price_cnt_10[5] = price_cnt_10[4]
        price_cnt_10[6] = price_cnt_10[5]
        price_cnt_10[7] = price_cnt_10[6]
        price_cnt_10[8] = price_cnt_10[7]
        price_cnt_10[9] = price_cnt_10[8]

        for i in range(1,10+1):
            price_cnt_accu = price_cnt_accu + price_cnt_10[i-1]
    ...
    ## Stop Flag
    if (price_cnt_accu <= -4*coin_num and stop_flag == 0):
        stop_flag = 1

    if (price_cnt_accu > 0 and stop_flag == 1):
        stop_flag = 0

    ## Strategy
    for i in range(1,coin_num+1):
        time.sleep(0.1)
        kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], current_price[i-1], cash, btc[i-1] = strategy(ticker_list[i-1], kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], low_mean[i-1], high_min[i-1], low_min[i-1], load_day, cash, btc[i-1], stop_flag)
    ...
    print('Now Time:', now.month, '.', now.day, '.', now.hour, '/ cash:', cash + cash_temp, '/ state:', state_sum, '/ coin num:', coin_num,'/ cnt accu:', price_cnt_accu, '/ stop flag:', stop_flag)
