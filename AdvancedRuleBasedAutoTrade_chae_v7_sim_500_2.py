...
def strategy(ticker, kkk, hhh, state, buy_price, low_mean, high_min, low_min, load_day, cash, btc, buy_price_origin, rate, rate_sum):
        ...
        if (current_price < low_mean and current_price > high_min):
            if (rate_sum >= 10):
                if cash > 205000:
                    ...
                    kkk = 0
                elif cash <= 205000 and cash > 5000:
                    krw = cash
                    btc = btc + (0.9995*krw/current_price) 
                    cash = cash - krw
                    state = 1
                ...
                if cash > 105000:
                    ...
                    buy_price_origin = current_price
                    kkk = 0
            elif (rate_sum < 0 and rate_sum > -10):
                if cash > 5000:
                    krw = 5000
                    btc = btc + (0.9995*krw/current_price) 
                    cash = cash - krw
...
while (not (now.year == 2022 and now.month == 2 and now.day == 15)):
    ...
    for i in range(1,coin_num+1):
        time.sleep(0.1)
        kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], current_price[i-1], cash, btc[i-1], buy_price_origin[i-1], rate[i-1] = strategy(ticker_list[i-1], kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], low_mean[i-1], high_min[i-1], low_min[i-1],load_day, cash, btc[i-1], buy_price_origin[i-1], rate[i-1], rate_sum)
