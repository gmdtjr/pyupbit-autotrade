...
ini_flag = 0
print("Sim Start")
while (not (now.year == 2022 and now.month == 5)):
#for iii in range(3):
#while (0):
    ...
    for i in range(1,coin_num+1):
        if (state[i-1] == 1):
            buy_val_min = buy_val_min + min(1000000, buy_val[i-1])
    # Time Feedback
    day_pre = now.day
    # Print
    print('Now Time:', now.month, '.', now.day, '.', now.hour, '.', now.minute, ',', now.second,'/ cash:', cash + cash_temp)
    print('state sum:', state_sum, '/ valid num:', valid_sum, '/ coin num:', coin_num, '/ buy val sum:', buy_val_sum, '/ buy val min:', buy_val_min)
