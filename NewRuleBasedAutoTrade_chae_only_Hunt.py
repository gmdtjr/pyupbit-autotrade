...
coin_num = 1
ticker_list = ["KRW-HUNT"]
tic_list = ["HUNT"]
ratio = 1*np.ones(coin_num)
...
while True:
    try:
        ...
        for i in range(1,coin_num+1):
            kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], current_price[i-1], low_mean[i-1], high_min[i-1], low_min[i-1] = strategy(ticker_list[i-1], tic_list[i-1], kkk[i-1], hhh[i-1], state[i-1], buy_price[i-1], ratio[i-1])
        ...
        for i in range(1,coin_num+1):
            print(tic_list[i-1],'-','st:',state[i-1],'/k:',kkk[i-1],'/cp:',current_price[i-1],'/mp:',round(low_mean[i-1],1),'/hp:',high_min[i-1],'/lp:',low_min[i-1])
