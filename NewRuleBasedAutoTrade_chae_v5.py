...
def strategy(ticker, tic, kkk, hhh, state, buy_price, buy_price_origin, ratio, low_mean, high_min, low_min, low_1, low_2, low_3):
    ...
    if (state == 1):
        ...
        if (current_price < low_min or 100*(current_price - buy_price_origin)/buy_price_origin < -2):
            btc = get_balance(tic)
            if btc > 0:
                upbit.sell_market_order(ticker, btc*0.9995)
            state = 0
...
while True:
    try:
        ...
        for i in range(1,coin_num+1):
            print(tic_list[i-1],'-','st:',state[i-1],'/k:',kkk[i-1],'/ratio:',ratio[i-1],'/bp:',buy_price[i-1],'/bpo:',buy_price_origin[i-1],'/cp:',current_price[i-1],'/mp:',round(low_mean[i-1],1),'/hp:',high_min[i-1],'/lp:',low_min[i-1])
