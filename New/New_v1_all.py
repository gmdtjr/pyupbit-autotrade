    ...
    if (state == 0 and hhh >= 1 and stop_flag == 0):
        if (current_price < low_mean and current_price > high_min and active_state == 1 and buy_value != 0):
            if cash > buy_value + 5000:
                ...
                cash = cash - krw
            elif cash <= buy_value + 5000 and cash > 10000:
                krw = cash
                btc = btc + (0.9995*krw/current_price) 
                cash = cash - krw
            state = 1
            ...
            kkk = 0
        elif (current_price < low_mean and current_price > high_min):
            state = 1
            buy_price = current_price
            buy_price_origin = current_price
            kkk = 0
...
stop_flag = 0
net1 = np.zeros(coin_num)
net2 = np.zeros(coin_num)
net3 = np.zeros(coin_num)
net4 = np.zeros(coin_num)
net5 = np.zeros(coin_num)
...
while (1):
    ...
    for i in range(1,coin_num+1):
        ...
        if (net_temp != -1000):
            ...
            net4[i-1] = net3[i-1]
            net5[i-1] = net4[i-1]
            net_sum[i-1] = net1[i-1] + net2[i-1] + net3[i-1] + net4[i-1] + net5[i-1]
    ...
    active_state_sum = 0
    for i in range(1,coin_num+1):
        active_state_sum = active_state_sum + active_state[i-1]

    print('Now Time:', now.month, '.', now.day, '.', now.hour, '.', now.minute, '/ cash:', cash + cash_temp, '/ state:', state_sum, '/ act state:', active_state_sum, '/ coin num:', coin_num,'/ cnt accu:', price_cnt_accu_sum, '/ stop flag:', stop_flag)
