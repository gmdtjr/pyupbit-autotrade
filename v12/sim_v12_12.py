...
while (1):
    ...
    ## Stop Flag
    if (price_cnt_accu_sum <= -10 and stop_flag == 0):
        stop_flag = 1
    if (price_cnt_accu_sum >= 2 and stop_flag == 1):
        stop_flag = 0
    ...
    print('Now Time:', now.month, '.', now.day, '.', now.hour, '.', now.minute, '/ cash:', cash + cash_temp, '/ state:', state_sum, '/ coin num:', coin_num,'/ cnt accu:', price_cnt_accu_sum, '/ stop flag:', stop_flag)
